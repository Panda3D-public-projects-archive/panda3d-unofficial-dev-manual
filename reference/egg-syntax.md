...
characters including skeletons, morphs, and multiple-joint assignments,
and character animation tables.
...
texture. If the texture is already a cube map texture, the
...
calculated for `CollisionPolygon`s.
...
fixed order that all geometry in the same bin will be rendered, from
...
be specified for `PointLight`s, as well as `draw-order`, plus one
additional attribute valid only for `PointLight`s and Lines:
...
If this is specified, then the thickness, above, is to be interpreted
...
`PandaNode`s in the scene graph (although the egg loader reserves the
...
is the responsibility of the show code to perform the switching. If
...
Sets the `CollideMask`s on the collision nodes and geometry nodes
...
Specifies that a special blend mode should be applied to geometry at
...
column of the table; `<V>` is a list of numbers of length
`length(contents) * num_frames`. Each letter of the contents string corresponds to a
...
any particular structure, so long as named entries like `VertexPool`s
and `Texture`s appear before they are referenced.
...
throughout the various `<Table>`s should define the same number of
